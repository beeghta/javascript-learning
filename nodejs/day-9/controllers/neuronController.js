...
import {
    ...
    searchNeurons as searchNeuronsService,
    deleteNeuron as deleteNeuronService,
    getNeuronCount as getNeuronCountService
...
const getAllNeurons = asyncHandler(async (request, response) => {
    ...
    const neurons = await getAllNeuronsService(page, limit);
    const total = await getNeuronCountService();
...
const searchNeurons = asyncHandler(async (request, response) => {
    const minActivity = Number(request.query.minActivity);
    const sort = request.query.sort;

    const neurons = await searchNeuronsService(
        minActivity,
        sort
    );
